...
Got from: http://jorisvr.nl/files/graphmatching/20090731/matching.py

*********************************************************************
Modified by felippe because of the homework's requirement for a minimum
perfect matching
*********************************************************************
...
_all__=['min_weight_matching']

def min_weight_matching(G, maxcardinality=True):
    """Compute a minimum-weighted matching in the undirected, weighted graph G.
    
    If maxcardinality is True, compute the maximum-cardinality matching
    with minimum weight among all minimum-cardinality matchings.
	...
	# Declara o minimo weigth como infinito
    minweight = 1000000000000000000
    allinteger = True
    for (i, j, wt) in G.todas_arestas():
        if i != j and wt < minweight:
            minweight = wt
    ...
    dualvar = dict(zip(gnodes, repeat(minweight)))
    ...
    def slack(v, w):
	#diferente do código que econtra o máximo, para encontrar o mínimo
	#a conta é feita ao contrário e o abs pega o módulo dessa subtração
	#então o slack (distância do nó para o maior) fica condizente para
	#encontrar o menor
	return abs(dualvar[v] + dualvar[w] - 2 * G.peso_aresta(v,w))
    ...
    def verifyOptimum():
        ...
        for (i, j, wt) in G.todas_arestas():
            ...
            #Esta conta do slack tem que ser feita ao contrário
            #uma vez que estamos trabalhando para achar o mínimo
            #o abs garante o módulo, assim so trabalhamos com positivos
            s = abs(dualvar[i] + dualvar[j] - 2 * wt)
            iblossoms = [ i ]
            jblossoms = [ j ]
        ...
        # all blossoms with positive dual value are full.
    ...
    while 1:
	stage += 1
	#print "Stage number = {0}".format(stage)
        ...
        substage = 0
        while 1:
	    #print "\tStage = {0} substage = {1}".format(stage,substage)
	    substage += 1
	    ...
	    #print "Delta usado = {0} deltatype = {1}".format(delta,deltatype)
    ...
    # Adding weights to solution and removing entries with same meaning
    lista = mate.keys()
    for key in lista:
	del lista[lista.index(mate[key])]
	del mate[mate[key]]
	mate[key] = (mate[key], G.peso_aresta(key,mate[key]))
