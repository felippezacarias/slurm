/*****************************************************************************\
 *  select_linear_colocation.c - node selection plugin for simple one-dimensional
...
#include "src/plugins/select/linear_colocation/select_linear_colocation.h"
 ...
 */
const char plugin_name[]       	= "Linear node selection plugin for colocation";
const char plugin_type[]       	= "select/linear_colocation";
const uint32_t plugin_id	= SELECT_PLUGIN_LINEAR_COLOCATION;
			      ...
			      bool remove_all)
{
	...
	if (_rem_tot_job(cr_ptr, job_ptr->job_id) == 0) {
		info("select/linear_colocation: job %u has no resources allocated",
		     job_ptr->job_id);
		return SLURM_ERROR;
	}
		       ...
		       struct job_record *to_job_ptr)
{
	...
	xassert(to_job_ptr);
	if (cr_ptr == NULL) {
		error("select/linear_colocation: cr_ptr not initialized");
		return SLURM_ERROR;
	}

	if (from_job_ptr->job_id == to_job_ptr->job_id) {
		error("select/linear_colocation: attempt to merge job %u with self",
		      from_job_ptr->job_id);
		return SLURM_ERROR;
	}
	...
	if ((from_job_resrcs_ptr == NULL) ||
		...
		error("select/linear_colocation: job %u lacks a job_resources struct",
		      from_job_ptr->job_id);
		return SLURM_ERROR;
	}
	...
	for (max_run_job=0; ((max_run_job<max_share) && (rc != SLURM_SUCCESS));
		...
		for (sus_jobs=0; ((sus_jobs<5) && (rc != SLURM_SUCCESS));
			...
			if ((j == prev_cnt) || (j < min_nodes))
				continue;
			prev_cnt = j;
			//COLOCATION: Sharing only for pairs scheduled by the colocation plugin
			//if (max_run_job > 0) {
			     ...
			     bitstr_t *exc_core_bitmap)
{
	...
	if (job_ptr->details->core_spec != NO_VAL16) {
		verbose("select/linear_colocation: job %u core_spec(%u) not supported",
